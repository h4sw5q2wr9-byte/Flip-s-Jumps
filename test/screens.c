...
int main(void) {
    ...
    /* One of each kind, spread out and labelled by position. */
    w->platforms[0] = (Platform){.x = 6,  .y = 14, .type = PlatformTypeNormal};
    w->platforms[1] = (Platform){.x = 34, .y = 14, .type = PlatformTypeMoving, .vx = 1.0f};
    w->platforms[2] = (Platform){.x = 62, .y = 14, .type = PlatformTypeBreakable};
    w->platforms[3] = (Platform){.x = 92, .y = 14, .type = PlatformTypeSpring};
    w->platforms[4] = (Platform){.x = 92, .y = 30, .type = PlatformTypeSpring, .spring_frame = 5};
    w->platforms[5] = (Platform){.x = 6,  .y = 30, .type = PlatformTypeBreakable, .broken = true};
    /* Player mid-fall, facing right; enemy alongside. */
    w->player.x = 34; w->player.y = 36; w->player.vy = 2.0f; w->player.facing_left = false;
    w->enemy.active = true; w->enemy.x = 60; w->enemy.y = 36;
    ...
    for(int i = 0; i < PLATFORM_COUNT; i++) w->platforms[i].y = 999;
    w->platforms[0] = (Platform){.x = 0, .y = 30, .type = PlatformTypeNormal};
    w->player.x = 124; w->player.y = 18; w->player.dying = false;
    w->enemy.active = false;
    game_draw(NULL, &app);
    screen_dump("PLAYER WRAPPING THE RIGHT EDGE");

    w->player.x = 60; w->player.dying = true;
    game_draw(NULL, &app);
